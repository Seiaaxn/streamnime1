// pages/explorer/AllDonghuaPage.jsx
import { useState, useEffect } from 'react';
import { Star, Loader2, Clapperboard } from 'lucide-react';
import axios from 'axios';

const API_BASE = 'https://anime-api-iota-beryl.vercel.app/api';

const AllDonghuaPage = ({ onAnimeSelect }) => {
    ...
    useEffect(() => {
        const fetchDonghua = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${API_BASE}/donghua/list?page=1`);
                setList(res.data.donghua || res.data.anime || []);
    ...
    const loadMore = async () => {
        ...
        try {
            const res = await axios.get(`${API_BASE}/donghua/list?page=${nextPage}`);
            setList(prev => [...prev, ...(res.data.donghua || res.data.anime || [])]);
            setHasMore(res.data.nextPage !== null);
            setPage(nextPage);
        } catch (error) {
    ...
    return (
        <div className="p-4">
            <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                    <h2 className="text-base font-bold text-white">All Donghua</h2>
                    <p className="text-xs text-gray-500">Browse all Chinese animation</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-12">
                    <p className="text-gray-500 text-sm">No donghua found</p>
                </div>
            ) : (
                <>
                    <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                        {list.map((item, index) => (
                            ...
                            >
                                <div className="relative aspect-[3/4] rounded-[7px] overflow-hidden mb-2 bg-dark-card">
                                    <img
                                        src={item.image || 'https://via.placeholder.com/300x400/1a1a1a/666666?text=No+Image'}
                                        alt={item.title}
                                    ...
                                    {item.score && item.score !== 'N/A' && (
                                        <div className="absolute top-1.5 right-1.5 flex items-center gap-1 bg-black/60 px-1.5 py-0.5 rounded">
                                            ...
                                            <span className="text-[10px] font-semibold text-white">{item.score}</span>
                                        </div>
                                    )}
                                    {/* Donghua badge */}
                                    <div className="absolute top-1.5 left-1.5 bg-red-500/80 px-1.5 py-0.5 rounded text-[9px] font-bold text-white">
                                        CN
                                    </div>
                                </div>
                                ...
                                <p className="text-[10px] text-gray-500 mt-0.5">{item.type || 'Donghua'}</p>
                            </div>
                        ))}
                    </div>
                    {hasMore && (
